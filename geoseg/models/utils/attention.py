...
from geoseg.models.utils import common
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        return (x + weight_value).contiguous()


class ContextEnhancementModule(nn.Module):
    def __init__(self, in_chan, out_chan):
        super(ContextEnhancementModule, self).__init__()
        self.conv_in = nn.Conv2d(in_chan, out_chan, kernel_size=3, stride=1, padding=1)
        self.attention = Attention(out_chan)
        ...
        self.conv_out = nn.Conv2d(out_chan * 2, out_chan, kernel_size=3, stride=1, padding=1)

    def forward(self, x):
        x = self.conv_in(x)

        x_att = self.attention(x)
        x_conv = self.conv1(x) + self.conv2(x)

        x = torch.cat((x_att, x_conv), 1)
