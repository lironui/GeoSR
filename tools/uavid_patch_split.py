...
def parse_args():
    ...
    parser.add_argument("--input-dir", default="data/uavid/train_sr")
    parser.add_argument("--output-img-dir", default="data/UAVid_2x/train_sr/images")
    parser.add_argument("--output-ref-dir", default="data/UAVid_2x/train_sr/references")
    parser.add_argument("--output-mask-dir", default="data/UAVid_2x/train_sr/masks")
...
def patch_format(inp):
    (input_dir, seq, imgs_output_dir, refs_output_dir, masks_output_dir, mode, split_size, stride, scale) = inp
    img_paths = glob.glob(os.path.join(input_dir, str(seq), 'references', "*.png"))
        ...
        for m in range(len(image_list)):
            ...
            for y in range(0, img.shape[0], stride[0]):
                for x in range(0, img.shape[1], stride[1]):
                    width, height = y, x
                    if y + split_size[0] > img.shape[0]:
                        width = img.shape[0] - split_size[0]
                    if x + split_size[1] > img.shape[1]:
                        height = img.shape[1] - split_size[1]

                    img_tile_cut = img[width:width + split_size[0], height:height + split_size[1]]
                    mask_tile_cut = mask[width:width + split_size[0], height:height + split_size[1]]
                    ...
                    if img_tile.shape[0] == split_size[0] and img_tile.shape[1] == split_size[1] \
                        ...
                        if mode == 'train':
                            out_ref_path = os.path.join(refs_output_dir, "{}_{}_{}_{}.png".format(seq, id, m, k))
                            out_img_path = os.path.join(imgs_output_dir, "{}_{}_{}_{}.png".format(seq, id, m, k))

                            img_tile = cv2.cvtColor(img_tile, cv2.COLOR_RGB2BGR)
                            cv2.imwrite(out_ref_path, img_tile)

                            low_res = cv2.resize(img_tile, (0, 0), fx=1 / scale, fy=1 / scale,
                                                    interpolation=cv2.INTER_CUBIC)

                            cv2.imwrite(out_img_path, low_res)
...
if __name__ == "__main__":
    ...
    refs_output_dir = args.output_ref_dir
    ...
    inp = [(input_dir, seq, imgs_output_dir, refs_output_dir, masks_output_dir, mode, split_size, stride, scale)
           for seq in seqs]
